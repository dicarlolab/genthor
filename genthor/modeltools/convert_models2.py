...
def main():
    ...
    model_path = os.path.join(os.environ["HOME"], "jxx_work", "jxx_models")

    # Temporary path in which to extract .obj files before conversion.
    tmp_path = os.path.join(os.environ["HOME"], "jxx_work", "jxx_tmp", "scrap")

    # Blender script and conversion command.
    #blender_path = os.path.join(os.environ["HOME"], "bin", "blender")
    #blender_path = os.path.join(os.environ["HOME"], 'Desktop', 'blender-2.63a-release-OSX_10.6_x86_64', 'blender.app', 'Contents', 'MacOS', 'blender')
    blender_path = os.path.join(os.environ["HOME"], 'dev', 'blender-2.63a-linux-glibc27-x86_64', 'blender')
    blender_script_name = "obj2egg.py"
    blender_command_base = "%s -b -P %s --" % (blender_path,
                                               blender_script_name)

    # Destination directory for .egg files
    egg_root_path = os.path.join(os.environ["HOME"], "jxx_work", "jxx_tmp", "processed_models")
