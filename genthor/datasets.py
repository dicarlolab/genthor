...
class WhiteChecker():
    ...
    def __call__(self, I):
        ...
        """
        if I.ndim == 3:
            assert I.shape[2] == 3, 'Must be L or RGB to check white, and check for pixel mask'
        elif I.ndim != 2:
            raise ValueError('Must be L or RGB to check white, and check for pixel mask')
        if I.ndim == 2:
            if I.dtype =='float32':
                white = 1
            elif I.dtype == 'uint8':
            ...
            if I.dtype =='float32':
                ...
                raise ValueError, 'data type of image not recognized'
            mask = np.uint8(np.all(I==white, 2))
        return mask
